...
export default function App () {
  const { error, invite, messages, clearError, reset, start, addMessage, addBlindPeer } = useWorklet()

  const [blindPeerKeyInput, setBlindPeerKeyInput] = useState('')
  ...
  const onAddBlindPeer = () => {
    if (initializing) {
      alert('Still initializing, please try later')
      return
    }
    addBlindPeer(blindPeerKeyInput.trim())
  }
  ...
  const renderChatRoom = () => (
    <>
      ...
      <Text>Add Blind Peer</Text>
      <View style={styles.inputRow}>
        <TextInput
          style={styles.input}
          value={blindPeerKeyInput}
          onChangeText={setBlindPeerKeyInput}
          placeholder='Type your blind peer listening key...'
        />
        <Button title='Add' onPress={onAddBlindPeer} />
      </View>
      <View style={styles.inputRow}>
        <TextInput
          style={styles.input}
      ...
      <ScrollView style={styles.messages}>
        {messages.map((msg, idx) => (
          <Text key={idx} style={styles.message}>{`${msg.text} ~ ${msg.info.name} ~ ${new Date(msg.info.at)} ~ ${msg.id}`}</Text>
        ))}
      </ScrollView>
    </>
  )
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <View style={styles.container}>
        ...
        {!invite ? renderSetupRoom() : renderChatRoom()}
      </View>
